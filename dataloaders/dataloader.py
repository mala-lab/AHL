...
class initDataloader():
    ...
    def build(self, ref = None, **kwargs):
        if self.args.dataset == "mvtecad":
            train_set = mvtecad.MVTecAD(self.args, train=True, AHL=False)
            test_set = mvtecad.MVTecAD(self.args, train=False, AHL=False)
            ...
            if self.args.AHL == True and ref == False:
                ...
                for j in range(self.args.episode_num):
                    ...
                    item1 = mvtecad.MVTecAD(self.args, train=True, AHL=True, sp=True, k=j, cluster_index = cluster_index)
                    item2 = mvtecad.MVTecAD(self.args, train=True, AHL=True, sp=False, k=j, cluster_index = cluster_index)
